...
function App() {
	...
	const [manualAbstentions, setManualAbstentions] = useState<Set<string>>(
		new Set()
	);
	const [autoAbstentions, setAutoAbstentions] = useState<Set<string>>(
		new Set()
	);
	const [useAbstentions, setUseAbstentions] = useState<boolean>(true);
	const [autoAbstainString, setAutoAbstainString] =
		useState<string>("I don't know");
	...
	const selectedQuestion = selectedRun?.results.find(
		(q) => q.query_id === selectedQuestionId
	);
				...
				>
					...
					>
						...
						{activeTab === "inspector" &&
							(selectedQuestion ? (
								<QuestionInspector
									...
									abstained={combinedAbstentions.has(
										selectedQuestion.query_id
									)}
									onToggleAbstention={(qid, val) =>
										setManualAbstentions((prev) => {
									...
									onChangeAutoAbstainString={
										setAutoAbstainString
									}
								/>
							) : (
								<div className="p-6 text-center">
									<div className="text-gray-500">
										...
										<p className="mb-4">
											Select a question to inspect its
											details.
										</p>
										<p className="text-sm">
											You can click on questions from the
											Metrics tab or use the dropdown
											below.
										</p>
									</div>
								</div>
						...
						{activeTab === "chunks" && (
							<ChunkAnalysis
								questions={selectedRun.results}
								onViewContextChange={(vc) => setChunksVC(vc)}
							/>
						)}
					</div>
				</div>
